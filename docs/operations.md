...
Scripts present in `scripts/` are intentionally split between root entrypoints and support files under `scripts/support/`.

| Script | Scope | Purpose | Inputs | Outputs | Dependencies | Referenced by |
| --- | --- | --- | --- | --- | --- | --- |
| `scripts/setup.ps1` | canonical root entrypoint | Restore repository dependencies in locked mode. | `-ForceEvaluate` optional restore refresh. | NuGet restore state under the configured package cache and `artifacts/obj`. | `dotnet`, `OnlyWinget.sln`, `scripts/install.ps1`. | README, operations docs. |
| `scripts/dev.ps1` | canonical root entrypoint | Launch the built WPF app, optionally building first. | `-Configuration`, `-Build`, `-NoRestore`, `-StopRunningInstance`. | Running `OnlyWinget.exe` process. | Built app output, `scripts/run.ps1`. | README, operations docs. |
| `scripts/check.ps1` | canonical root verification gate | Reproduce CI locally through the supported sequential checks. | `-Configuration`, `-RunWingetSmoke`. | Build report, test results, app build output, setup EXE, internal MSIs. | `scripts/gate.ps1`, `dotnet`, tests, WiX tools. | GitHub Actions, README, operations docs. |
...
| `scripts/lint.ps1` | canonical root entrypoint | Run PowerShell script lint. | `-Required` fails when PSScriptAnalyzer is missing. | Console analyzer report or not_available warning. | `scripts/lint-scripts.ps1`, optional `PSScriptAnalyzer`. | README, operations docs. |
...
| `scripts/install.ps1` | compatibility entrypoint | Restore repository dependencies in locked mode. | `-ForceEvaluate` optional restore refresh. | NuGet restore state under the configured package cache and `artifacts/obj`. | `dotnet`, `OnlyWinget.sln`, `scripts/support/ScriptHelpers.ps1`. | `scripts/setup.ps1`, compatibility calls. |
| `scripts/build.ps1` | root entrypoint | Build the WPF app; also supports warning-as-error verification. | `-Configuration`, `-NoRestore`, `-StopRunningInstance`, `-WarnAsError`. | App build output under `artifacts/bin/OnlyWinget/<Configuration>/net8.0-windows/`. | `dotnet`, app csproj, `scripts/support/ScriptHelpers.ps1`. | `scripts/typecheck.ps1`, `scripts/gate.ps1`, operations docs. |
| `scripts/run.ps1` | compatibility entrypoint | Launch the built WPF app, optionally building first. | `-Configuration`, `-Build`, `-NoRestore`, `-StopRunningInstance`. | Running `OnlyWinget.exe` process. | Built app output, optional `scripts/build.ps1`, `scripts/support/ScriptHelpers.ps1`. | `scripts/dev.ps1`, compatibility calls. |
...
| `scripts/package.ps1` | root entrypoint | Build architecture-specific internal MSIs and the unified setup EXE. | `-Configuration`, `-Version`, `-NoRestore`, `-StopRunningInstance`, `-Architecture`, `-SkipBundle`. | `artifacts/dist/OnlyWinget/<Configuration>/*-setup.exe` and `msi/*.msi`. | `dotnet`, WiX 3.14 tools, app and setup sources, `scripts/support/ScriptHelpers.ps1`. | `scripts/gate.ps1`, `scripts/check.ps1`, operations docs, PROJECT_STATUS. |
...
| `scripts/gate.ps1` | compatibility verification gate | Reproduce CI locally through the supported sequential checks. | `-Configuration`, `-RunWingetSmoke`. | Build report, test results, app build output, setup EXE, internal MSIs. | `dotnet`, `scripts/build.ps1`, `scripts/package.ps1`, `scripts/lint-scripts.ps1`, tests, WiX tools, `scripts/support/ScriptHelpers.ps1`. | `scripts/check.ps1`, compatibility calls. |
| `scripts/lint-scripts.ps1` | compatibility maintenance entrypoint | Run PSScriptAnalyzer over active PowerShell scripts, or report script lint as `not_available` when the optional module is absent. | `-Required` fails when PSScriptAnalyzer is missing. | Console analyzer report or not_available warning. | Optional `PSScriptAnalyzer`, `scripts/support/PSScriptAnalyzerSettings.psd1`. | `scripts/lint.ps1`, `scripts/gate.ps1`, compatibility calls. |
| `scripts/support/PSScriptAnalyzerSettings.psd1` | support config | Configure script analysis rules. | Analyzer invocation. | Rule exclusions consumed by PSScriptAnalyzer. | PSScriptAnalyzer. | `scripts/lint-scripts.ps1`. |
